...
mod tests {
    ...
    fn invalid_char_error() {
        ...
        if let HexToBytesError(ToBytesError::InvalidChar(e)) = error {
            assert_eq!(format!("{}", e), "invalid hex char 71 at pos 2");
    ...
    fn odd_length_string_error() {
        ...
        let error = result.unwrap_err();
        assert_eq!(format!("{}", error), "odd length, failed to create bytes from hex");
        check_source(&error);
        if let HexToBytesError(ToBytesError::OddLengthString(e)) = error {
            assert_eq!(format!("{}", e), "odd hex string length 3");
    ...
    fn invalid_length_error() {
        ...
        let error = result.unwrap_err();
        assert_eq!(format!("{}", error), "failed to parse hex");
        check_source(&error);
        if let HexToArrayError(ToArrayError::InvalidLength(e)) = error {
            assert_eq!(format!("{}", e), "invalid hex string length 3 (expected 8)");
